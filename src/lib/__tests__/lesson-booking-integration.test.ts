 ...
 *
 * Uses the REAL database via test dummy accounts:
 * - dummy.pro@golflessons.be (pro_profile_id=5, slug=dummy-pro)
 * - dummy.student@golflessons.be (user_id=10, member role)
 *
...
const DUMMY_PRO_PROFILE_ID = 5; // dummy.pro@golflessons.be
const DUMMY_PRO_LOCATION_ID = 4; // Test Golf Club
const DUMMY_STUDENT_USER_ID = 10; // dummy.student@golflessons.be
...
beforeAll(async () => {
  ...
  // Ensure the dummy pro has availability templates for all weekdays (Mon-Fri 09:00-17:00)
  const existingTemplates = await db
    .select({ id: proAvailability.id })
    .from(proAvailability)
    .where(eq(proAvailability.proProfileId, DUMMY_PRO_PROFILE_ID));

  if (existingTemplates.length === 0) {
    // Create Mon(0), Tue(1), Thu(3), Fri(4) — skip Wed(2) for tests that expect no Wednesday availability
...
describe("database connectivity", () => {
  ...
  it("can read test user", async () => {
    const [user] = await db
      .select({ id: users.id, email: users.email })
      .from(users)
      .where(eq(users.id, DUMMY_STUDENT_USER_ID))
      .limit(1);

    expect(user).toBeDefined();
    expect(user.email).toContain("dummy.student");
  });
...
describe("getAvailableDates (DB integration)", () => {
  it("returns dates within booking horizon", async () => {
    ...
    const maxDate = format(
      addDays(new Date(), profile.bookingHorizon),
      "yyyy-MM-dd",
    );
    for (const d of dates) {
  ...
  it("only returns dates matching availability template days", async () => {
    ...
    // Fetch the actual template days from DB
    const templates = await db
      .select({ dayOfWeek: proAvailability.dayOfWeek })
      .from(proAvailability)
      .where(
        and(
          eq(proAvailability.proProfileId, DUMMY_PRO_PROFILE_ID),
          eq(proAvailability.proLocationId, DUMMY_PRO_LOCATION_ID),
        ),
      );
...
describe("getAvailableSlots (DB integration)", () => {
  ...
  it("returns no slots for a day with no template (Wednesday)", async () => {
    // Find the next Wednesday (ISO dayOfWeek=2)
    const today = new Date();
    let cursor = today;
    while (true) {
...
describe("booking creation", () => {
  it("can create and clean up a test booking", async () => {
    ...
    const [booking] = await db
      ...
      .values({
        proProfileId: DUMMY_PRO_PROFILE_ID,
        bookedById: DUMMY_STUDENT_USER_ID,
        proLocationId: DUMMY_PRO_LOCATION_ID,
        date: date,
...
describe("back-to-back booking", () => {
  it("cancelled booking frees the slot for new bookings", async () => {
    ...
    expect(
      slotsAfterBook.find((s) => s.startTime === slot.startTime),
    ).toBeUndefined();
    ...
    await db
      ...
      .set({
        ...
        cancellationReason: "[TEST] Test cancellation",
      })
    ...
    expect(
      slotsAfterCancel.find((s) => s.startTime === slot.startTime),
    ).toBeDefined();
  });
...
describe("concurrent booking / slot re-validation", () => {
  it("second booking for same slot sees it as taken", async () => {
    ...
    const matchingSlot = slotsAfter.find(
