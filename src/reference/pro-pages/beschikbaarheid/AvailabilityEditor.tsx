...
import { saveWeeklyTemplate, saveWeekOverrides, saveWeekOverridesWithCancellations } from "./actions";
...
const ROWS = (GRID_END_HOUR - GRID_START_HOUR) * 2; // 26 half-hour rows
const CELL_H = 22;
...
export default function AvailabilityEditor({
  ...
  useEffect(() => {
    if (prevAvailRef.current !== availability) {
      ...
      setTemplateGrid(availabilityToLocationGrid(availability)); // eslint-disable-line react-hooks/set-state-in-effect
    }
  }, [availability]);
  ...
  if (locations.length === 0) {
    return (
      <div className="mt-8 rounded-xl border border-dashed border-amber-300 bg-amber-50 p-8 text-center text-sm text-amber-800">
        Geen locaties gekoppeld.{" "}
        <Link href="/pro/profiel" className="text-gold-600 underline hover:text-gold-500">
          Voeg locaties toe via je profiel
        </Link>{" "}
        om beschikbaarheid in te stellen.
      </div>
    );
  }
...
function WeeklyTemplateGrid({
  ...
  // Auto-save dirty locations after 1s of inactivity
  useEffect(() => {
  ...
  return (
    <div className="rounded-xl border border-green-200 bg-white p-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-display text-lg font-semibold text-green-950">Weekschema</h2>
          <p className="mt-0.5 text-xs text-green-700/50">
            Selecteer een locatie en teken beschikbaarheid in. Klik nogmaals om te wissen.
          </p>
        </div>
        <div className="flex items-center gap-3">
          {saveStatus === "saving" && (
            <span className="text-xs text-green-700/50 animate-pulse">Opslaan...</span>
          )}
          {saveStatus === "saved" && (
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        <span className="text-xs text-green-700/50">Locatie:</span>
        {locations.map((loc) => {
...
function PreviewBlockingGrid({
  ...
  useEffect(() => {
    if (key !== prevKey.current) {
      prevKey.current = key;
      setBlockedCells(initBlockedCells(days, overrides)); // eslint-disable-line react-hooks/set-state-in-effect
  ...
  const { templateAvailMap, bookingMap } = useMemo(() => {
    ...
    for (const b of bookings) {
      ...
      const label = (b.bookerName || "Geboekt") + (b.participantCount > 1 ? ` (+${b.participantCount - 1})` : "");
  ...
  // ─── Build override records from current state ──
  ...
  function findConflictingBookings(): SerializedBooking[] {
    const found: SerializedBooking[] = [];
    const seen = new Set<number>();

    for (const b of bookings) {
      if (b.status?.startsWith("cancelled") || seen.has(b.id)) continue;
      const dayIdx = days.findIndex((d) => d.date === b.date);
      if (dayIdx < 0 || days[dayIdx].isPast) continue;

      const startRow = timeToRow(b.startTime);
      const endRow = timeToRow(b.endTime);

      // Check if any row of this booking is blocked
      if (fullDayBlocked[dayIdx]) {
        found.push(b);
        seen.add(b.id);
        continue;
  ...
  // ─── Auto-save overrides after 1s of inactivity ──
  useEffect(() => {
    if (!dirty) return;
    clearTimeout(debounceRef.current);
    debounceRef.current = setTimeout(() => {
      const { datesToReplace, records } = buildOverrideRecords();
      const conflicting = findConflictingBookings();

      if (conflicting.length > 0) {
        // Pause auto-save and show confirmation
        setConflict({
          bookingIds: conflicting.map((b) => b.id),
          bookings: conflicting,
          pendingSave: { datesToReplace, overrides: records },
        });
        setDirty(false);
        return;
      }
    ...
    return () => clearTimeout(debounceRef.current);
  }, [dirty, days, blockedCells, fullDayBlocked, extraAvailCells, reasons]); // eslint-disable-line react-hooks/exhaustive-deps

  // ─── Conflict dialog handlers ─────────────────────
  function handleConfirmCancellations() {
    if (!conflict) return;
    setCancelling(true);
    setSaveStatus("saving");

    (async () => {
      const result = await saveWeekOverridesWithCancellations({
        ...conflict.pendingSave,
        bookingIdsToCancel: conflict.bookingIds,
      });
      setCancelling(false);
      setConflict(null);
      if (!result.error) {
  ...
  return (
    <div className="rounded-xl border border-green-200 bg-white p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-display text-lg font-semibold text-green-950">Planning & Blokkades</h2>
          <p className="mt-0.5 text-xs text-green-700/50">
            Teken blokkades of extra beschikbaarheid. Dubbelklik een blokkade voor een reden.
          </p>
        </div>
        <div className="flex items-center gap-3">
          {saveStatus === "saving" && (
      ...
      {/* Brush selector */}
      <div className="mt-3 flex flex-wrap items-center gap-2">
        <span className="text-xs text-green-700/50">Modus:</span>
        <button
      ...
      >
        ...
        >
          ...
          {/* ── Hele dag row: label + checkboxes + reason ── */}
            ...
            return [
              ...
              ...Array.from({ length: 7 }, (_, dayIdx) => {
                ...
                if (isBlocked) {
                  ...
                  title += ` · Geblokkeerd${reason ? ` — ${reason}` : ""}`;
                } else if (hasExtra) {
      ...
      <div className="mt-4 flex flex-wrap gap-4 border-t border-green-100 pt-3">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-green-700/60">Geboekt</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="h-3 w-5 rounded-sm" style={{ backgroundColor: "rgba(239, 68, 68, 0.45)" }} />
          <span className="text-[10px] text-green-700/60">Geblokkeerd</span>
        </div>
      </div>
          ...
          >
            ...
            <div className="flex gap-1.5">
              <input
                ...
                placeholder="bijv. vakantie, afwezig..."
      ...
      {conflict && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="mx-4 w-full max-w-md rounded-xl border border-green-200 bg-white p-6 shadow-xl">
            <h3 className="text-lg font-semibold text-green-950">
              Bestaande boekingen gevonden
            </h3>
            <p className="mt-2 text-sm text-green-800">
              De blokkade overlapt met {conflict.bookings.length === 1 ? "een bestaande boeking" : `${conflict.bookings.length} bestaande boekingen`}. Als je doorgaat worden deze automatisch geannuleerd en krijgen de leden een annulatie-e-mail.
            </p>
            ...
            <div className="mt-5 flex justify-end gap-2">
              ...
              >
                {cancelling ? "Bezig..." : "Blokkeren & annuleer boekingen"}
              </button>
            </div>
          </div>
        </div>
