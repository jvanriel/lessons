 ...
 * matching the availability editor grid ["Ma","Di","Wo","Do","Vr","Za","Zo"].
...
export function buildCancelIcs(params: IcsParams): string {
  ...
  const uid = `booking-${bookingId}@silverswing.golf`;

  return [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//Silver Swing//Lesson Booking//NL",
    "CALSCALE:GREGORIAN",
    "METHOD:CANCEL",
    "BEGIN:VEVENT",
    `UID:${uid}`,
    ...
    "STATUS:CANCELLED",
    "SEQUENCE:1",
    `ORGANIZER;CN=Silver Swing:mailto:${process.env.GMAIL_SEND_AS || "noreply@silverswing.golf"}`,
    "END:VEVENT",
    "END:VCALENDAR",
  ].join("\r\n");
}
...
export function buildIcs(params: IcsParams): string {
  ...
  return [
    ...
    "TRIGGER:-PT1H",
    "ACTION:DISPLAY",
    "DESCRIPTION:Golfles over 1 uur",
    "END:VALARM",
    "END:VEVENT",
    "END:VCALENDAR",
  ].join("\r\n");
