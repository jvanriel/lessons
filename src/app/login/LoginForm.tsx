...
function GoogleErrorMessage({ error, locale }: { error: string | null; locale: Locale }) {
  if (!error?.startsWith("google_")) return null;
  const message = error === "google_no_account"
    ? t("auth.googleNoAccount", locale)
    : t("auth.googleError", locale);
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center text-xs">
            <span className="bg-green-950 px-3 text-green-100/40">
              {t("auth.or", locale)}
            </span>
          </div>
        </div>
        ...
        <a
          href={googleUrl}
          className="flex w-full items-center justify-center gap-3 rounded-lg border border-green-700 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50"
        >
          <GoogleIcon />
