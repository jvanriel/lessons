...
export default function BottomNav({ roles }: BottomNavProps) {
  ...
  // Determine which tabs to show: pro tabs take priority if user has pro role
  const tabs = roles.includes("pro") ? proTabs : memberTabs;

  return (
    <nav className="flex h-14 shrink-0 items-center justify-around border-t border-green-200 bg-white md:hidden">
      {tabs.map((tab) => {
        const active =
